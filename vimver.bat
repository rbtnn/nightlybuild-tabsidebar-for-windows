...
set VIMSCRIPT=join(filter(map(readfile('version.h'),{_,s-^>matchstr(s,'^#define VIM_VERSION_\(MAJOR\^|MINOR\)\s\+\zs\d\+$')}),'!empty(v:val)') + filter(map(readfile('version.c'),{_,s-^>printf('%%04d',matchstr(s,'^\s\+\zs\d\+\ze,$'))}),'str2nr(v:val)')[:0], '_')
